...
{
    public partial class RemainingPayment : Page
    {
        ...
        protected void btnViewPayment_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(planName))
            {
                ...
                lblRemainingPayment.Text = "";
                return;
            }

            GetRemainingPayment(mobileNo, planName);
        }

        private void GetRemainingPayment(string mobileNo, string planName)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                try
                {
                    ...
                    SqlCommand command = new SqlCommand("SELECT dbo.Remaining_plan_amount(@mobile_num, @plan_name)", connection);
                    ...
                    {
                        lblRemainingPayment.Text = $"Remaining Payment Amount: {remainingAmount}";
                        lblMessage.Text = "";
                    }
                    else
                    {
                        lblRemainingPayment.Text = "No remaining payment found for the specified account and plan.";
                        lblMessage.Text = "";
                    }
                }
                catch (Exception ex)
